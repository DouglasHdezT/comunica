...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
        <?php
 $theme_locations = get_nav_menu_locations(); $menu_obj = get_term( $theme_locations['secciones'], 'nav_menu' ); $menu_name = $menu_obj->name; $items = wp_get_nav_menu_items($menu_name); query_posts(''); for($i=0;$i
<count($items);$i++){ $flag=t rue; if ( have_posts() ) { while ( have_posts() ) { the_post(); if(in_category($items[$i]->title)){ if($flag){ $url = nameToUrl($items[$i]->title);?>
